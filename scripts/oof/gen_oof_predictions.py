...
Usage example:
  .\.venv\Scripts\python.exe scripts/oof/gen_oof_predictions.py --model vgg19 --folds 5 \
    --phase1-epochs 8 --batch-size 16 --lr 1e-4 --weight-decay 1e-4

This script implements staged transfer learning per-fold:
  Phase1: freeze backbone, train classifier head only.
  Phase2: unfreeze last feature block, fine-tune with LR/10 (optional via --phase2).

OOF outputs:
  outputs/{model}_oof_probs.npy   # shape (N, C)
  outputs/{model}_oof_meta.csv    # index,file_path,label
  outputs/checkpoints/{model}_fold{fold}_phase1_best.pt
"""
import argparse
import random
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(N), y)):
        ...
        # Save fold checkpoint (formal name: include phase1 label because default uses Phase-1)
        ckpt_path = ckpt_dir / f"{args.model}_fold{fold+1}_phase1_best.pt"
